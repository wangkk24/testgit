package com.pukka.ydepg.view.loadingball;
...
public class MonIndicator extends View {
	...
	private int[] colors = new int[]{this.getContext().getResources().getColor(R.color.loading_yellow), this.getContext().getResources().getColor(R.color.loading_green), this.getContext().getResources().getColor(R.color.loading_blue)};
	...
	private static int flashTime = 800;//加载动画执行时间,单位ms
	...
	public MonIndicator(Context context, AttributeSet attrs) {
		...
		a.recycle();
		//int cycle = a.getInt(R.styleable.monindicator_cycle, 500);//周期，默认为2秒
		int cycle = flashTime;
		cycle = cycle/2;
...
//		drawCircle05(canvas);
    ...
    private void drawCircle02(Canvas canvas){
    	...
    	int totalWidth = this.circleRadius * 2 * this.wrappers.size() + (this.wrappers.size()-1) * this.circleSpacing;
    	int centerX = this.getWidth()/2 - totalWidth/2 + (wrapper.diameter + this.circleSpacing) + this.circleRadius;
    	int centerY = this.getHeight()/2;
