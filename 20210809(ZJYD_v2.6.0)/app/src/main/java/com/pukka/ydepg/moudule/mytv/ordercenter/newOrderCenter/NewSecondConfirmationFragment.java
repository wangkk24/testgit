...
public class NewSecondConfirmationFragment extends BaseMvpFragment<MyPresenter> implements MyContact.IMyView, View.OnKeyListener, View.OnClickListener {


    /*是否是使用VRS接口*/
    public final static boolean IS_USE_VRS = true;
    ...
    protected int attachLayoutRes() {
        return R.layout.fragment_second_confirmation_new;
    }
    ...
    public void onResume() {
        ...
        if (null != presenter) {
            if (IS_USE_VRS){
                presenter.queryUserOrderingSwitch();
            }else{
...
//                switch_locktv.requestFocus();
    ...
    //VRS短信验证成功，儿童锁开关设置为关闭
    @Override
    public void checkVerifiedCodeSuccess() {
        presenter.changeUserOrderingSwitch(0,getActivity());
    }
    ...
    public boolean onKey(View v, int keyCode, KeyEvent event) {
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            switch (keyCode) {
                case KeyEvent.KEYCODE_DPAD_LEFT:
                    if (v.getId() == R.id.send_btn || v.getId() == R.id.comfirm_btn) {
                        return true;
                    }
                    return false;
                case KeyEvent.KEYCODE_DPAD_UP:
                    ...
                    if (v.getId() == R.id.comfirm_btn){
                        if (et_input_verification.isFocusable()){
                            et_input_verification.requestFocus();
                        }else if (send_btn.isFocusable()){
                            send_btn.requestFocus();
                        }else if (switch_locktv.isFocusable()){
                            switch_locktv.requestFocus();
                        }
                        return true;
                    }
        ...
        switch (v.getId()) {
            ...
            case R.id.comfirm_btn:
                ...
                if (null != mProfile) {
                    ...
                    if (IS_USE_VRS){
                        presenter.checkVerifiedCode(verification,billId,getActivity());
                    }else{
    ...
    public void switchLock() {
        if (currentStateClose()) {
            if (IS_USE_VRS){
                presenter.changeUserOrderingSwitch(1,getActivity());
            }else{
                UpdateSubscriberRequest request = new UpdateSubscriberRequest();
    ...
    public void countDown(int time) {
        ...
        if (null != mProfile) {
            ...
            if (IS_USE_VRS){
                presenter.sendVerifiedCode(billId,getActivity());
            }else{
