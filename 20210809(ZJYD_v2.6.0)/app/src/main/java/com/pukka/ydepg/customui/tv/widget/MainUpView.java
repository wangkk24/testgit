...
import com.pukka.ydepg.customui.tv.bridge.BaseEffectBridge;
import com.pukka.ydepg.customui.tv.bridge.OpenEffectBridge;
...
public class MainUpView extends FrameLayout {
	...
	/**
	 * 将MainUpView控件添加到指定的View视图层中
	 */
	public void attach2View(ViewGroup rootView) {
		ViewGroup.LayoutParams layoutParams = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
				ViewGroup.LayoutParams.WRAP_CONTENT);
		rootView.addView(this, layoutParams);
		rootView.setClipChildren(false);
	...
	private void initEffectBridge() {
		...
		//BaseEffectBridge baseEffectBridge = new RecyclerViewBridge();
	...
	public void setFocusView(View newView, View oldView, float scale) {
		setFocusView(newView, oldView, scale, scale);
...
//		setFocusView(view, scale, scale);
	 ...
	 *            Y放大比例
	 */
	public void setFocusView(View view, View oldView, float scaleX, float scaleY) {
		if (this.mEffectBridge != null)
			this.mEffectBridge.onFocusView(view, oldView, scaleX, scaleY);
	}
	 ...
	 * 隐藏移动焦点框
	 * */
	public void move2FocusByInvisible(View view){
		this.mEffectBridge.setUpRectDrawable(null);
		this.setFocusView(view,1.0f);
	}
