...
public class NewVoddetailRecListAdapter extends RecyclerView.Adapter <NewVoddetailRecListAdapter.ViewHolder> {

    private static final String TAG = "NewVoddetailRecListAdap";
    ...
    public NewVoddetailRecListAdapter(Context context, List<VOD> list, int index, boolean hasEpisodes) {
        ...
        public ViewHolder(View itemView) {
            ...
            itemView.setOnFocusChangeListener(new View.OnFocusChangeListener() {
                ...
                public void onFocusChange(View v, boolean hasFocus) {
                    ...
                    if (hasFocus){
                        if (index == 0 && position < 6){
                            //滚动到第二屏
                            EventBus.getDefault().post(new VoddetailScrollEvent(2));
                        }else if (index == 0 && position >= 6){
                            //滚动到第三屏
                            EventBus.getDefault().post(new VoddetailScrollEvent(3));
                        }else if (index == 1){
                            //滚动到最后
                            EventBus.getDefault().post(new VoddetailScrollEvent(4));
                        }
...
//                            }
            ...
            itemView.setOnKeyListener(new View.OnKeyListener() {
                ...
                public boolean onKey(View v, int keyCode, KeyEvent event) {
                    ...
                    if (event.getAction() == KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_DPAD_UP && position <= 5 && index == 0) {

                        if (hasEpisodes){
                            EventBus.getDefault().post(new VoddetailScrollEvent(VoddetailScrollEvent.EpisodeListRequesetFocus));
                        }else{
                            EventBus.getDefault().post(new VoddetailScrollEvent(VoddetailScrollEvent.PlayViewRequesetFocus));
                        }
                        return true;
                    }
    ...
    public interface onClickListenr{
        void onClick(boolean ismigu, VOD vod);
    }
